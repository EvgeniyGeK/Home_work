# Домашняя работа по курсу Python-разработчик, разделы 8.1 - 11.2
...
* Модуль utils содержит функцию read_json_file, которая принимает на вход путь до JSON-файла и возвращает список словарей с данными о финансовых транзакциях.
* Модуль external_api содержит функцию money_transaction, которая принимает на вход транзакцию и возвращает сумму транзакции в рублях.  Если транзакция была в валюте происходит обращение к внешнему API для получения текущего курса валют и конвертации суммы операции в рубли.
* Модуль loggers.py с функцией логирования.
* Модуль utils_csv_exel, который содержит функции read_exel и read_csv
* Модуль search_of_dict с функцией process_bank_search для поиска в списке словарей операций по строке статус операции функцией process_bank_operations для подсчета количества банковских операций определенного типа.
* Модуль sorted_dict_main с функцией sorted_dict, которая представляет собой пользовательское меню сортировки
* Модуль print_transaction_info с функцией print_transactions, которая предназначена для вывода результата сортировки пользователю.
* Модули с тестами:
...
* Модуль test_utils - тесты работы функции read_json_file.
* Модуль test_external_api - тесты работы функции money_transaction.
* Модуль test_utils_csv_exel - содержит тесты на функции read_exel и read_csv
* Модуль test_search_of_dict - содержит тесты для функций process_bank_search и process_bank_operations
* Модуль test_sorted_dict_main - содержит тесты для функции sorted_dict
* Модуль test_print_transaction_info - содержит тесты для функции print_transactions
* 
...
* Функция read_json_file принимает на вход путь до файла и возвращает список словарей. В случае ошибки возвращает пустой список.
* Функция money_transaction, которая принимает на вход транзакцию и возвращает сумму транзакции в рублях.  Если транзакция была в валюте происходит обращение к внешнему API для получения текущего курса валют и конвертации суммы операции в рубли.
* Функция get_logger логирует модули masks_card и utils.
* Функция read_exel преобразует данные из файла excel в список словарей.
* Функция read_csv преобразует данные из файла csv в список словарей.
* Функция search_of_dict осуществляет поиск транзакций по строке статус операции, который вводит пользователь.
* Функция process_bank_operations осуществляет подсчет заданных операций используя Count.
* Функция sorted_dict позволяет пользователю настроить сортировку запрашиваемой информации по дате в зависимости от условий, по возрастанию или убыванию.
* Функция print_transactions выводит информацию для пользователя с учетом сортировки, при этом маскируя номера счетов и карт.

Сборка кода осуществлена в модуле main/
